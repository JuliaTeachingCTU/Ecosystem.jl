...
let
    ...
    world = PSDU.World(vcat([sheep,sheep2], [PSDU.Grass(i) for i=2:3000]))
    ...
    SUITE["ParametricSpeciesDictUnion"]["reproduce"] = @benchmarkable PSDU.reproduce!($sheep, $world)
end

let
    sheep = PST.Sheep(1,1,1,1,1,:female)
    sheep2 = PST.Sheep(3001,1,1,1,1,:male)
    world = PST.World(vcat([sheep,sheep2], [PST.Grass(i) for i=2:3000]))
...
let
    ...
    world = PSSDU.World(vcat([sheep,sheep2], [PSSDU.Grass(i) for i=2:3000]))
    @info "Benching..." typeof(world) PSSDU.find_food(sheep,world)
    SUITE["ParametricSpeciesSexDictUnion"]["find_food"] = @benchmarkable PSSDU.find_food($sheep, $world)
    SUITE["ParametricSpeciesSexDictUnion"]["reproduce"] = @benchmarkable PSSDU.reproduce!($sheep, $world)
end

let
    sheep = PSST.Sheep(1,1,1,1,1,PSST.Female)
    sheep2 = PSST.Sheep(3001,1,1,1,1,PSST.Male)
    world = PSST.World(vcat([sheep,sheep2], [PSST.Grass(i) for i=2:3000]))
    ...
    SUITE["ParametricSpeciesSexNTuple"]["reproduce"] = @benchmarkable PSST.reproduce!($sheep, $world)
end
